...
class SafeOptimizer:
    """
    Chapter 16.6: SafeOpt
    """
    def __init__(self, gp, X_domain, y_max, beta=3.0):
        self.gp = gp
        self.X = X_domain # Không gian rời rạc
        self.y_max = y_max
        self.beta = beta
        self.m = len(X_domain)
        
        # Các vector biên và tập hợp
    ...
    def get_new_query_point(self, visited_indices=None):
        """
        Section 16.6: Chọn điểm tiếp theo
        Có tham số visited_indices để tránh chọn lại điểm cũ
        """
        # Hợp của tập M và E
        ...
        # Chọn điểm có width = u - l lớn nhất
        candidate_indices = np.where(candidates)[0]
        w = self.u - self.l
        
        # Tìm index có width lớn nhất
